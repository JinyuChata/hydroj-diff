...
export default function (
  env: { watch?: boolean; production?: boolean; measure?: boolean } = {},
) {
  ...
  function postcssLoader() {
    return {
      ...
      options: {
        postcssOptions: { sourceMap: false, config: root("postcss.config.js") },
      },
    };
  }
  ...
  const config: import("webpack").Configuration = {
    ...
    plugins: [
      ...
      new webpack.IgnorePlugin({
        resourceRegExp: /(^\.\/locale$|mathjax|abcjs|vditor.+\.d\.ts)/,
      }),
      new CopyWebpackPlugin({
        patterns: [
          { from: root("static") },
          {
            from: root("components/navigation/nav-logo-small_dark.png"),
            to: "components/navigation/nav-logo-small_dark.png",
          },
          {
            from: root(
              `${dirname(require.resolve("streamsaver/package.json"))}/mitm.html`,
            ),
            to: "streamsaver/mitm.html",
          },
          {
            from: root(
              `${dirname(require.resolve("streamsaver/package.json"))}/sw.js`,
            ),
            to: "streamsaver/sw.js",
          },
          {
            from: root(
              `${dirname(require.resolve("vditor/package.json"))}/dist`,
            ),
            to: "vditor/dist",
          },
          {
            from: root(
              `${dirname(require.resolve("graphiql/package.json"))}/graphiql.min.css`,
            ),
            to: "graphiql.min.css",
          },
          {
            from: `${dirname(require.resolve("monaco-themes/package.json"))}/themes`,
            to: "monaco/themes/",
          },
        ],
      }),
      new webpack.DefinePlugin({
        "process.env.VERSION": JSON.stringify(
          require("@hydrooj/ui-default/package.json").version,
        ),
      }),
      ...
      new webpack.NormalModuleReplacementPlugin(
        /\/(vscode-)?nls\.js/,
        require.resolve("../../components/monaco/nls"),
      ),
      new webpack.NormalModuleReplacementPlugin(
        /^prettier[$/]/,
        root("../../modules/nop.ts"),
