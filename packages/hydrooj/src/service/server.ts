...
export class HandlerCommon {
  ...
  async limitRate(
    ...
    withUserId = system.get("limit.by_user")
  ) {
  ...
  renderTitle(str: string) {
    ...
    if (this.UiContext.extraTitleContent)
      return `${this.translate(str)} - ${this.UiContext.extraTitleContent} - ${name}`;
  ...
  checkPerm(...args: bigint[]) {
    if (!this.user.hasPerm(...args)) {
      if (this.user.hasPriv(PRIV.PRIV_USER_PROFILE))
        throw new PermissionError(...args);
...
export class Handler extends HandlerCommon {
  ...
  binary(data: any, name?: string) {
    ...
    if (name)
      this.response.disposition = `attachment; filename="${encodeRFC5987ValueChars(name)}"`;
  }

  // This is beta API, may be changed in the future.
  progress(message: string, params: any[]) {
    Hydro.model.message.sendInfo(
      this.user._id,
    ...
    if (
      ...
      try {
        ...
        if (host !== this.request.host)
          this.context.pendingError = new CsrfTokenError(host);
      } catch (e) {
        this.context.pendingError = new CsrfTokenError();
      }
    }
    if (!argv.options.benchmark) await this.limitRate("global", 5, 100);
    if (!this.noCheckPermView && !this.user.hasPriv(PRIV.PRIV_VIEW_ALL_DOMAIN))
  ...
  async onerror(error: HydroError) {
    ...
    if (
      this.user?._id === 0 &&
      (error instanceof PermissionError || error instanceof PrivilegeError)
    ) {
      this.response.redirect = this.url("user_login", {
        query: {
          redirect:
            (this.context.originalPath || this.request.path) +
            this.context.search,
        },
      });
    } else {
      this.response.status =
        error instanceof UserFacingError ? error.code : 500;
...
async function handle(ctx: KoaContext, HandlerClass, checker) {
  ...
  try {
    ...
    const operation =
      method === "post" && ctx.request.body?.operation
        ? `_${ctx.request.body.operation}`.replace(/_([a-z])/gm, (s) =>
            s[1].toUpperCase()
          )
        : "";
    ...
    const steps = [
      ...
      ...(operation
        ? [
            `handler/before-operation/${name}`,
            "handler/before-operation",
            `post${operation}`,
            "log/__operationDone",
          ]
        : []),
    ...
    while (current < steps.length) {
      ...
      if (control) {
        ...
        if (index <= current) {
          logger.warn(
            "Returning to previous step is not recommended:",
            step,
            "->",
            control
          );
    ...
    try {
      await serial(
        `handler/error/${HandlerClass.name.replace(/Handler$/, "")}`,
        h,
        e
      );
...
export function Route(
  ...
  router.all(name, path, (ctx) =>
    handle(ctx as any, RouteHandler, Checker(permPrivChecker))
  );
...
export class ConnectionHandler extends HandlerCommon {
  ...
  onerror(err: HydroError) {
    ...
    if (
      ...
      !(
        (err instanceof PrivilegeError || err instanceof PermissionError) &&
        this.user?._id === 0
      )
    ) {
      logger.error(
        `Path:${this.request.path}, User:${this.user?._id}(${this.user?.uname})`
      );
      logger.error(err);
    }
    this.send({
...
export function Connection(
  ...
  router.ws(prefix, async (conn, _, ctx) => {
    const { args, request, response, user, domain, UiContext } =
      ctx.HydroContext;
    const h = new RouteConnHandler(
      ctx,
      args,
      request,
      response,
    ...
    try {
      ...
      for (const { name, target } of h.__subscribe || [])
        disposables.push(bus.on(name, target.bind(h)));
...
export class RouteService extends Service {
  ...
  private register(
    func: typeof Route | typeof Connection,
  ...
  public withHandlerClass(
    name: string,
    callback: (HandlerClass: typeof HandlerCommon) => any
  ) {
...
export async function apply(pluginContext: Context) {
  ...
  const corsAllowHeaders =
    "x-requested-with, accept, origin, content-type, upgrade-insecure-requests";
  app.use(async (ctx, next) => {
    if (ctx.request.headers.origin) {
      const host = new URL(ctx.request.headers.origin).host;
      if (
        host !== ctx.request.headers.host &&
        `,${system.get("server.cors")},`.includes(`,${host},`)
      ) {
    ...
    if (ctx.request.search.toLowerCase().includes("x-amz-credential"))
  ...
  if (process.env.DEV) logger.info("Running in dev mode. Cache is disabled.");
  for (const addon of [...global.addons].reverse()) {
    const dir = resolve(addon, "public");
    if (!fs.existsSync(dir)) continue;
    app.use(
      cache(dir, {
        maxAge: process.env.DEV ? 0 : 24 * 3600 * 1000,
      })
    );
  }
  if (process.env.DEV) {
...
${ctx.response.status} ${endTime - startTime}ms ${ctx.response.length}`);
  ...
  app.use(
    Body({
      ...
      formidable: {
        uploadDir,
        maxFileSize:
          parseMemoryMB(system.get("server.upload") || "256m") * 1024 * 1024,
        keepExtensions: true,
      },
    })
  );
  ...
  const layers = [
    baseLayer,
    rendererLayer(router, logger),
    responseLayer(logger),
    userLayer,
  ];
  app
    .use(async (ctx, next) => await next().catch(console.error))
    .use(domainLayer);
  ...
  wsServer.on("connection", async (socket, request) => {
    ...
    await domainLayer(ctx, () =>
      baseLayer(ctx, () => layers[1](ctx, () => userLayer(ctx, () => {})))
    );
