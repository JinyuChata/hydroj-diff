...
class RecordListHandler extends ContestDetailBaseHandler {
  ...
  @param("filterStatus", Types.Boolean, true) // 反转status筛选逻辑
  ...
  async get(
    ...
    let tdoc = null;
    let invalid = false;
    if (origin === 0) this.response.template = "record_main.html";
    else this.response.template = "record_main_backup.html";
    ...
    if (tid) {
      ...
      if (
        !contest[
          q.uid === this.user._id ? "canShowSelfRecord" : "canShowRecord"
        ].call(this, tdoc, true)
      ) {
        throw new PermissionError(PERM.PERM_VIEW_CONTEST_HIDDEN_SCOREBOARD);
      }
    ...
    if (this.user.hasPriv(PRIV.PRIV_VIEW_JUDGE_STATISTICS) && !full) {
      this.response.body.statistics = await record.stat(
        allDomain ? undefined : domainId,
      );
    }
  }
}
...
class RecordDetailHandler extends ContestDetailBaseHandler {
  ...
  async get(
    ...
    if (rdoc.contest?.toString().startsWith("0".repeat(23))) {
      ...
      canView ||=
        contest.canShowSelfRecord.call(this, this.tdoc, true) &&
        rdoc.uid === this.user._id;
      if (!canView && rdoc.uid !== this.user._id)
    ...
    canViewCode ||=
      this.user.hasPerm(PERM.PERM_READ_RECORD_CODE_ACCEPT) &&
      self?.status === STATUS.STATUS_ACCEPTED;
    if (this.tdoc) {
      const tsdoc = await contest.getStatus(
        domainId,
        this.tdoc.docId,
        this.user._id,
      );
      ...
      if (!tsdoc?.attend && !problem.canViewBy(pdoc, this.user))
        throw new PermissionError(PERM.PERM_VIEW_PROBLEM_HIDDEN);
    } else if (pdoc && !problem.canViewBy(pdoc, this.user))
      throw new PermissionError(PERM.PERM_VIEW_PROBLEM_HIDDEN);
    if (!canViewCode) {
      rdoc.code = "";
      rdoc.files = {};
    ...
    if (origin === 0) this.response.template = "record_detail.html";
    else this.response.template = "record_detail_backup.html";
  ...
  async postRejudge(domainId: string, rid: ObjectId) {
    ...
    if (!pdoc?.config || typeof pdoc.config === "string")
      throw new ProblemConfigError();
    const priority = await record.submissionPriority(this.user._id, -20);
    const isContest =
      this.rdoc.contest &&
      this.rdoc.contest.toString() !== "000000000000000000000000";
...
class RecordMainConnectionHandler extends ConnectionHandler {
  ...
  async prepare(
    ...
    if (tid) {
      ...
      if (pretest || contest.canShowScoreboard.call(this, this.tdoc, true))
  ...
  async onRecordChange(rdoc: RecordDoc) {
    if (!this.allDomain) {
      ...
      if (!this.all) {
        if (
          rdoc.contest &&
          ![this.tid, "000000000000000000000000"].includes(
            rdoc.contest.toString(),
          )
        )
          return;
...
class RecordDetailConnectionHandler extends ConnectionHandler {
  ...
  async sendUpdate(rdoc: RecordDoc) {
    this.send({
      ...
      summary_html: await this.renderHTML("record_detail_summary.html", {
        rdoc,
        pdoc: this.pdoc,
      }),
    });
  ...
  async onRecordChange(rdoc: RecordDoc, $set?: any, $push?: any) {
    ...
    if (this.applyProjection && rdoc.input === undefined)
...
class RecordListApiHandler extends ContestDetailBaseHandler {
  tdoc?: Tdoc;
  ...
  @param("all", Types.Boolean)
  @param("allDomain", Types.Boolean)
  @param("objective", Types.Boolean, true)
  async get(
    domainId: string,
    page = 1,
    pid?: string | number,
...
export async function apply(ctx) {
  ...
  ctx.Route("record_list_api", "/api/record/list", RecordListApiHandler);
