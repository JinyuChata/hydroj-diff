...
registerResolver(
  ...
  async (arg, ctx) => {
    ...
    const udocs: User[] = arg.exact
      ? []
      : await user.getPrefixList(
        ctx.args.domainId,
        arg.search,
        Math.min(arg.limit || 10, 10),
      );
...
class UserLoginHandler extends Handler {
  ...
  async post(
    ...
    if (
      system.get("system.contestmode") &&
      !udoc.hasPriv(PRIV.PRIV_EDIT_SYSTEM)
    ) {
      if (udoc._loginip && udoc._loginip !== this.request.ip)
        throw new ValidationError("ip");
      if (system.get("system.contestmode") === "strict") {
        const udocs = await user
          .getMulti({loginip: this.request.ip, _id: {$ne: udoc._id}})
          .toArray();
...
class UserSudoHandler extends Handler {
  ...
  async post(domainId: string, password = "", tfa = "", authnChallenge = "") {
    ...
    if (this.user.authn && authnChallenge) {
      const challenge = await token.get(authnChallenge, token.TYPE_WEBAUTHN);
      if (challenge?.uid !== this.user._id)
        throw new InvalidTokenError(token.TYPE_TEXTS[token.TYPE_WEBAUTHN]);
      if (!challenge.verified) throw new ValidationError("challenge");
      await token.del(authnChallenge, token.TYPE_WEBAUTHN);
...
class UserWebauthnHandler extends Handler {
  ...
  async post({domainId, result}) {
    ...
    const parseId = (id: Binary) =>
      Buffer.from(id.toString("hex"), "hex").toString("base64url");
    const authenticator = udoc._authenticators?.find(
      (c) => parseId(c.credentialID) === result.id,
    );
...
export class UserRegisterAllHandler extends Handler {
  noCheckPermView = true;

  async get() {
    this.response.template = "user_register_all.html";
  }
}
export class UserLostPassAllHandler extends Handler {
  noCheckPermView = true;

  async get() {
    this.response.template = "user_lostpass_all.html";
  }
}
...
export class UserRegisterPhoneHandler extends Handler {
  noCheckPermView = true;

  async get() {
    this.response.template = "user_register_phone.html";
  }
  ...
  async post(domainId: string, mail: string, phoneNumber: string) {
    if (phoneNumber) {
      ...
      const id = String.random(10, "0123456789abcdefghijklmnopqrstuvwxyz");
      const sms_code = String.random(6, "0123456789");
      await token.add(
        token.TYPE_REGISTRATION,
        10 * Time.minute,
        {phone: phoneNumber, sms_code: sms_code},
        id,
      );
      // send sms
      // 准备参数
      const username = "codemate";
      const password = "2c82008051f946fb99a00e4db0033208";
      ...
      const params = {
        ...
        p: password,
        m: phoneNumber,
        c: `【codemate】您的验证码是${sms_code}，600s内有效`,
      };

      const url = `https://api.smsbao.com/sms?${querystring.stringify(params)}`;
      console.log(url);

      const response = await fetch(url);
      if (!response.ok) {
        throw new ValidationError("send error");
      }
      const responseData = await response.text();
      console.log(responseData);
      this.response.redirect = this.url("user_register_phone_with_code", {
        code: id,
      });

      // await global.Hydro.lib.sendSms(phoneNumber, 'register', id);
      // this.response.body = { phone: phoneNumber, id: id };
      // this.response.template = 'user_register_sms.html';
    } else throw new ValidationError("phone");
  }
}
...
class UserRegisterWithCodeHandler extends Handler {
  ...
  async get(domainId: string, code: string) {
    ...
    if (!tdoc)
      throw new InvalidTokenError(
        token.TYPE_TEXTS[token.TYPE_REGISTRATION],
        code,
      );
  ...
  async post(
    ...
    const tdoc = await token.get(code, token.TYPE_REGISTRATION);
    if (!tdoc || (!tdoc.mail && !tdoc.phone) || !(tdoc.mail_code == mail_code))
      throw new InvalidTokenError(
        token.TYPE_TEXTS[token.TYPE_REGISTRATION],
        code,
      );
    ...
    this.session.scope = PERM.PERM_ALL.toString();
    this.response.redirect =
      tdoc.redirect || this.url("homepage");
  }
}

class UserRegisterPhoneWithCodeHandler extends Handler {
  noCheckPermView = true;

  @param("code", Types.String)
  async get(domainId: string, code: string) {
    this.response.template = "user_register_phone_with_code.html";
    const tdoc = await token.get(code, token.TYPE_REGISTRATION);
  ...
  async post(
    ...
    const tdoc = await token.get(code, token.TYPE_REGISTRATION);
    if (!tdoc || (!tdoc.mail && !tdoc.phone))
      throw new InvalidTokenError(
        token.TYPE_TEXTS[token.TYPE_REGISTRATION],
        code,
      );
    if (password !== verify) throw new VerifyPasswordError();
    if (tdoc.phone) tdoc.mail = `mob-${tdoc.phone}@hydro.local`;
    if (tdoc.sms_code !== sms_code) throw new VerifyCodeError();
    const uid = await user.create(
      tdoc.mail,
      uname,
      password,
...
class UserLostPassHandler extends Handler {
  ...
  async post(domainId: string, mail: string) {
    ...
    const [tid] = await token.add(
      token.TYPE_LOSTPASS,
      10 * Time.minute,
      {mail: mail, mail_code: mail_code, uid: udoc._id},
    ...
    this.response.redirect = this.url("user_lostpass_with_code", {
      code: tid,
    });
...
class UserLostPassWithCodeHandler extends Handler {
  ...
  async get({domainId, code}) {
    ...
    if (!tdoc)
      throw new InvalidTokenError(token.TYPE_TEXTS[token.TYPE_LOSTPASS], code);
    const udoc = await user.getById(domainId, tdoc.uid);
  ...
  async post(
    ...
    const tdoc = await token.get(code, token.TYPE_LOSTPASS);
    if (!tdoc || tdoc.mail_code != mail_code)
      throw new InvalidTokenError(token.TYPE_TEXTS[token.TYPE_LOSTPASS], code);
...
class UserLostPassPhoneHandler extends Handler {
  noCheckPermView = true;

  async get() {
    this.response.template = "user_lostpass_phone.html";
  }

  @param("phone", Types.String)
  async post(domainId: string, phoneNumber: string) {
    const mail = `mob-${phoneNumber}@hydro.local`;
    const udoc = await user.getByEmail("system", mail);
    await Promise.all([
      this.limitRate(`send_sms_${phoneNumber}`, 60, 1, false),
      this.limitRate("send_sms", 3600, 15, false),
    ...
    const [tid] = await token.add(
      token.TYPE_LOSTPASS,
      10 * Time.minute,
      {mail: mail, mail_code: sms_code, uid: udoc._id},
      // system.get("session.unsaved_expire_seconds"),
      // {uid: udoc._id},
    );
...
class UserDetailHandler extends Handler {
  ...
  async get(domainId: string, uid: number) {
      ...
      await Promise.all(
        [domainId, ...(union?.union || [])].map(async (did) => {
          const psdocs = await problem
            .getMultiStatus(did, {uid, status: STATUS.STATUS_ACCEPTED})
            .toArray();
          pdocs.push(
            ...Object.values(
    ...
    if (this.user.hasPerm(PERM.PERM_VIEW_PROBLEM_SOLUTION)) {
      const psdocs = await SolutionModel.getByUser(domainId, uid)
        .limit(10)
        .toArray();
...
class OauthCallbackHandler extends Handler {
  ...
  async get(args: any) {
    if (!global.Hydro.module.oauth[args.type])
      throw new UserFacingError("Oauth type");
    const r = await global.Hydro.module.oauth[args.type].callback.call(
      this,
      args,
    );
...
class UserCenterHandler extends Handler {
  async get(domainId: string, uid: number) {
    this.response.template = "user_center.html";
  }
}
...
export async function apply(ctx) {
  ...
  ctx.Route(
    "user_register_all",
    "/register_all",
    UserRegisterAllHandler,
    PRIV.PRIV_REGISTER_USER,
  );
  ctx.Route(
    "user_lostpass_all",
    "/lostpass_all",
    UserLostPassAllHandler,
    PRIV.PRIV_REGISTER_USER,
  );
  ctx.Route(
    "user_register",
  ...
  ctx.Route(
    "user_register_phone_with_code",
    "/register_phone/:code",
    UserRegisterPhoneWithCodeHandler,
    PRIV.PRIV_REGISTER_USER,
  );
  ctx.Route(
    "user_logout",
  ...
  ctx.Route("user_lostpass_phone", "/lostpass_phone", UserLostPassPhoneHandler);
  ctx.Route(
